...
int main()
{
    ...
    //写日志对象
    LogFile log_file("/home/WebServer/Log/Server.txt");
    log_file.Start_Log();
    ...
    //建立一个Rpc服务器并注册服务
    SP_channel Listen(new RpcChannel(lsfd,EPOLLIN | EPOLLET,Maccept,NULL,&time_round));
    RpcChannel::service.RegisterService("Add",Add);
